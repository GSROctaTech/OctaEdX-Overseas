...
/*on change purchaseorder_id data get	*/
$(document).on("change", "#purchaseorder_id", function(){
	...
	var data = {
        action: 'ValidatePurchaseorderID',
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='USED')
			{	
				alert("Purchase Order No Is Already Exist!!");  
...
$(document).ready(function(){
	...
	$( "#product_purchaseorder_form" ).validate({
		rules: {
			field: {required: true},
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Purchase Order");
							}
	...
	$( ".product-combobox" ).autocomplete({
		...
		change: function(event, ui) {
			if (ui.item == null || ui.item == undefined) {
				...
				$($targetparnt).find(".quantity").removeAttr("max");
