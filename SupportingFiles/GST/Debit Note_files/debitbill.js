...
function InvoiceTypeSet(country,party_type = null){
	if ( $("#hidden-invoice-country").is( "select" ) ) {
		...
		{		
			if(party_type == 'SEZ'){
				$("#hidden-invoice-country").html('<option value="SEZDebitNote">Sez Debit Note (With IGST)</option><option value="SEZDebitNoteWithoutIGST">Sez Debit (Without IGST)</option>');
			}else{
				$("#hidden-invoice-country").html('<option value="Debit">Regular</option><option value="BillofSupply">Bill of Supply</option>');								
			}
		...
		{		
			$("#hidden-invoice-country").html('<option value="ExportDebitNote">Export Debit (With IGST)</option><option value="ExportDebitNoteWithoutIGST">Export Debit (Without IGST)</option>');						
...
function SetNoteType(){
	if ( $("#hidden-invoice-debitnotetype").is( "select" ) ) {
	
		if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Vendor")
		{
			$("#hidden-invoice-debitnotetype").html('<option value="GoodsReturn">Goods Return</option><option value="DiscountAfterSale">Discount After Sale</option>');
		}
		else if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Customer")
		{
			$("#hidden-invoice-debitnotetype").html('<option value="CorrectionInInvoice">Correction In Invoice</option>');
		}
		else if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Customer-Vendor")
		{
			$("#hidden-invoice-debitnotetype").html('<option value="GoodsReturn">Goods Return</option><option value="DiscountAfterSale">Discount After Sale</option><option value="CorrectionInInvoice">Correction In Invoice</option>');
		}
		else{
			$("#hidden-invoice-debitnotetype").html('');
		}
	}
	SetValidationRules();
}
function SetIGTS(){
	...
	if($("#hidden-invoice-country").val() == "SEZDebitNoteWithoutIGST" ||  $("#hidden-invoice-country").val() == "ExportDebitNoteWithoutIGST" || $("#hidden-invoice-country").val() == "BillofSupply")
	{
		DisableALL_IGST();
	}
	else if($("#hidden-invoice-country").val() == "SEZDebitNote" || $("#hidden-invoice-country").val() == "ExportDebitNote" )
	{
		EnableIGST();
	}
	else if($(".customer-information-invoice #hidden-invoice-state").val() == $(".customer-information-invoice .PlaceofSupply").val())
	{
		DisableIGST();
	}
	else
...
$(document).ready(function(){
	...
	$(document).on("change", "#hidden-invoice-debitnotetype", function(){
		SetValidationRules();
	});
...
{
	if($("#hidden-invoice-debitnotetype").val() == "GoodsReturn")
	{
		$( "#againstno" ).rules( "add", {required: true });
	}
	else
	{
		$( "#againstno" ).rules( "add", {required: false });
	}
}

/*on change debit_id data get	*/
$(document).on("change", "#debit_id", function(){
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='USED')
			{	
				alert("Debit Note No Is Already Exist!!");  
				$("#debit_id").val("");
				$("#debit_id").focus();
			}
		},
...
$(document).ready(function(){
	...
	$( "#product_quotation_form" ).validate({
		...
		submitHandler: function(form) {
			if(validateCurrentRowsForSubmit() && checkValidDate())
			{
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Debit Note");
							}
							$('#gogst_popup').popup({
								scrolllock: true,
								transition: 'all 0.3s'
		...
		change: function(event, ui) {
			if (ui.item == null || ui.item == undefined) {
				...
				if (ui.item) {
					...
					if(ui.item.purchaseprice != ui.item.customerprice)
					{
						$($targetparnt).find(".rate").val(ui.item.customerprice);
						$($targetparnt).find(".customer_rate_label").html(ui.item.purchaseprice);
						
					}
					else
					{
						$($targetparnt).find(".rate").val(ui.item.purchaseprice);
...
$(document).on("change", "#againstno", function(){
	
	var data = {
        action: 'GetAgainstNumber',
		invoice_id: $(this).val(),
		company_id: $("#customers").val(),
		note_type: $("#hidden-invoice-debitnotetype").val(),
        type: 'Debit'
    }
	$.ajax({  
		type: "POST",  
		...
		success: function(data){
			if(data.status=='OK')
			{	
				$("#against_date").val(data.bill_date);
				$("#hidden_invoice_detail_id").val(data.invoice_detail_id);
				
			}
			else if(data.status=='Error'){
				alert("Invoice Not Create By This User!!");  
				$("#againstno").val("");
				$("#against_date").val("");
				$("#againstno").focus();
			}
