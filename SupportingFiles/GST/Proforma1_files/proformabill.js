	...
	if ( $("#hidden-invoice-country").is( "select" ) ) {
		...
		{		
			if(party_type == 'SEZ'){
				...
				$("#hidden-invoice-country").html('<option value="Proforma">Regular</option><option value="BillofSupply">Bill of Supply</option>');								
			}
...
/*on change proforma_id data get	*/
$(document).on("change", "#proforma_id", function(){
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='USED')
			{	
				alert("Proforma No Is Already Exist!!");  
				$("#proforma_id").val("");
				$("#proforma_id").focus();
			}
		},
...
$(document).ready(function(){
	...
	$( "#product_proforma_form" ).validate({
		rules: {
			field: {required: true},
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Proforma Invoice");
							}
