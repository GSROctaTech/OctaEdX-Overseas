...
rowdata += '	</td>';
rowdata += '	<td>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" name="quantity[]" class="quantity decimal_numeric_only"  placeholder="Qty." class="span2" style="width:50px;" maxlength="10"><label>Qty</label></div>';
...
function InvoiceTypeSet(country,party_type = null){
	if ( $("#hidden-invoice-country").is( "select" ) ) {
		...
		{		
			if(party_type == 'SEZ'){
				$("#hidden-invoice-country").html('<option value="SEZCreditNote">Sez Credit Note (With IGST)</option><option value="SEZCreditNoteWithoutIGST">Sez Credit Note (Without IGST)</option>');
			}else{
				$("#hidden-invoice-country").html('<option value="Credit">Regular</option><option value="BillofSupply">Bill of Supply</option>');								
			}
		...
		{		
			$("#hidden-invoice-country").html('<option value="ExportCreditNote">Export Credit Note (With IGST)</option><option value="ExportCreditNoteWithoutIGST">Export Credit Note (Without IGST)</option>');						
...
function SetNoteType(){
	if ( $("#hidden-invoice-creditnotetype").is( "select" ) ) {
	
		if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Customer")
		{
			$("#hidden-invoice-creditnotetype").html('<option value="GoodsReturn">Goods Return</option><option value="DiscountAfterSale">Discount After Sale</option>');
		}
		else if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Vendor")
		{
			$("#hidden-invoice-creditnotetype").html('<option value="CorrectionInInvoice">Correction In Invoice</option>');
		}
		else if($("#customers option[value="+$("#customers").val()+"]").attr("data-type") == "Customer-Vendor")
		{
			$("#hidden-invoice-creditnotetype").html('<option value="GoodsReturn">Goods Return</option><option value="DiscountAfterSale">Discount After Sale</option><option value="CorrectionInInvoice">Correction In Invoice</option>');
		}
		else{
			$("#hidden-invoice-creditnotetype").html('');
		}
	}
	SetValidationRules();
}
function SetIGTS(){
	...
	if($("#hidden-invoice-country").val() == "SEZCreditNoteWithoutIGST" ||  $("#hidden-invoice-country").val() == "ExportCreditNoteWithoutIGST" || $("#hidden-invoice-country").val() == "BillofSupply")
	{
		DisableALL_IGST();
	}
	else if($("#hidden-invoice-country").val() == "SEZCreditNote" || $("#hidden-invoice-country").val() == "ExportCreditNote" )
	{
		EnableIGST();
	}
...
$(document).ready(function(){
	...
	$(document).on("change", "#hidden-invoice-creditnotetype", function(){
		SetValidationRules();
	});
...
{
	if($("#hidden-invoice-creditnotetype").val() == "GoodsReturn")
	{
...
/*on change credit_id data get	*/
$(document).on("change", "#credit_id", function(){
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='USED')
			{	
				alert("Credit Note No Is Already Exist!!");  
				$("#credit_id").val("");
				$("#credit_id").focus();
			}
		},
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Credit Note");
							}
...
$(document).on("change", "#againstno", function(){
	...
	var data = {
		...
		note_type: $("#hidden-invoice-creditnotetype").val(),
        type: 'Credit'
    }
	$.ajax({  
		type: "POST",  
