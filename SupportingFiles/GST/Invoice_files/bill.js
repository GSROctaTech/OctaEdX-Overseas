...
rowdata += '	</td>';
rowdata += '	<td>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" name="hsccode[]" class="hsccode"  placeholder="HSN/SAC" class="span2" style="width:60px;" maxlength="20"  '+SaleInvoiceWithoutPriceChange+'><label>HSN/SAC</label></div>';
rowdata += '	</td>';
rowdata += '	<td>';
...
rowdata += '</td>';
rowdata += '	<td><center>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" name="rate[]" class="rate decimal_numeric_only"    placeholder="Price" style="width:100px;"  class="span2" maxlength="10"  '+SaleInvoiceWithoutPriceChange+'><label>Price</label></div>';
rowdata += '		<div class="currency-fields" >';
rowdata += '		<div class="currency-factor-symbol">Rs</div>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" class="rate_usd decimal_numeric_only"    placeholder="Price" class="span2" maxlength="10"  '+SaleInvoiceWithoutPriceChange+'><label>Price</label></div>';
rowdata += '		</div>';
...
rowdata += '		<input type="hidden" name="taxable_line_value[]" class="taxable_line_value"   class="span2"   '+SaleInvoiceWithoutPriceChange+'>';
rowdata += '	</td>';
/* rowdata += '	<td>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" name="cgst[]" class="cgst decimal_numeric_only"    placeholder="%"  class="span2" style="width:30px;" maxlength="5" '+SaleInvoiceWithoutPriceChange+'><label>CGST</label></div>';
...
rowdata += '	</td>';
rowdata += '	<td>';
rowdata += '		<div class="inputlabel_wrapper"><input type="text" name="sgst[]" class="sgst decimal_numeric_only"    placeholder="%"  class="span2" style="width:30px;" maxlength="5"  '+SaleInvoiceWithoutPriceChange+'><label>SGST</label></div>';
...
rowdata += '	<div class="inputlabel_wrapper">';
rowdata += '	<select name="cgst_select" class="cgst_select"   '+SaleInvoiceWithoutPriceChange+'>';
...
rowdata += '	<input type="hidden" name="cgst[]" class="cgst decimal_numeric_only"    placeholder="%"  style="width:30px;" maxlength="5"  '+SaleInvoiceWithoutPriceChange+'>												';
rowdata += '	<input type="hidden" name="sgst[]" class="sgst decimal_numeric_only"    placeholder="%"  style="width:30px;" maxlength="5"  '+SaleInvoiceWithoutPriceChange+'>												';
rowdata += '</td>';
...
rowdata += '	</td>';
rowdata += '	<td>';
rowdata += '		<div class="inputlabel_wrapper inputlabel_wrapper_half"><input type="text" name="cess[]" class="cess decimal_numeric_only"    style="width:40px;" placeholder="%"   '+SaleInvoiceWithoutPriceChange+'><label>Cess %</label></div>';
rowdata += '		<input value="" type="hidden" name="cessrate[]" class="cessrate" '+SaleInvoiceWithoutPriceChange+'>';
rowdata += '		<center class="hide_unser_tab">+</center>';
rowdata += '		<div class="inputlabel_wrapper inputlabel_wrapper_half"><input type="text" name="cess_amount[]" class="cess_amount decimal_numeric_only"   placeholder="Rs"  style="width:40px;"   '+SaleInvoiceWithoutPriceChange+'><label>Cess Rs</label></div>';
rowdata += '	</td>';
rowdata += '	<td style="width:90px;" ><center>';
rowdata += '		<div class="inputlabel_wrapper">	<input type="text" name="line_total[]" class="line_total decimal_numeric_only"    placeholder="Total"  class="span2" style="width:80px;"  '+SaleInvoiceWithoutPriceChange+'><label>Total</label></div>';
rowdata += '		<div class="currency-fields" >';
rowdata += '		<div class="currency-factor-symbol">Rs</div>';
rowdata += '			<div class="inputlabel_wrapper"><input type="text" class="line_total_usd"    placeholder="Total"  class="span2" '+SaleInvoiceWithoutPriceChange+'><label>Total</label></div>';
rowdata += '		</div>';
rowdata += '	</center></td>';
rowdata += '	<td>';
rowdata += '		<center>';
...
function SetIGTS(){
	...
	if($("#hidden-invoice-country").val() == "ExportInvoice" ||  $("#hidden-invoice-country").val() == "ExportInvoiceWithoutIGST" || show_export_fields_for_all)
	{
		/* open export shippning detail code*/
		$("#shipping_data_drawer").addClass("open_data_drwawer");
	}
	else
	{
		/* close export shippning detail code*/
		$("#shipping_data_drawer").removeClass("open_data_drwawer");
	}
...
function checkValidPayment(){
	if($("#payment").val() == 'CASH' || $("#payment").val() == 'ONLINE' || $("#payment").val() == 'CHECK')
	{
		...
		if(pr == "")
		{
			alert("Enter valid payment received value.");
		...
		else
		{
			...
			{
				alert("Payment Received must be less then OR equal to Total Amount.");
				return false;
			}	
		}
	}
...
$(document).ready(function(){
	...
	$( "#product_invoice_form" ).validate({
		...
		submitHandler: function(form) {
			...
			if(validateCurrentRowsForSubmit() && checkValidDate() && checkValidPayment() && qty_validate_items())
			{
				$("#SameShippingAdd").attr("disabled", false);
				$("#customers").attr("disabled", false);
				$("#document-item-list [readonly]").attr("readonly", false);
				$.ajax({
					type: form.method,
					data: $(form).serialize(),
					dataType: 'json',
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Invoice");
							}
...
function UpdateCalculations(){
	...
	else
	{
		...
		var currency_cus_pay_grandtotal = Math.round(conver_amount_with_factor(cus_pay_grandtotal,currency_rate ) * 100) / 100;
		...
		$("#currency_cus_pay_grandtotal_lable").html(currencysymbol+ " " + currency_cus_pay_grandtotal);
		$("#currency_cus_pay_grandtotal").val(currency_cus_pay_grandtotal);
		
		$("#currency_total_in_words").html(number2text_with_currency(currency_cus_pay_grandtotal,currencycode));
		$("#hidden_currency_total_in_words").val(number2text_with_currency(currency_grandtotal,currencycode));
		$("#hidden_currency_cus_pay_total_in_words").val(number2text_with_currency(currency_cus_pay_grandtotal,currencycode));
	}
