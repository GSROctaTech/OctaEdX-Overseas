...
function allowOnlyNumber(evt){
	try {
		...
		if ((charCode != 46 || $(this).val().indexOf('.') != -1) && (charCode < 48 || charCode > 57)&&charCode!=8) {
...
function InvoiceTypeSet(country,party_type = null){
	if ( $("#hidden-invoice-country").is( "select" ) ) {
		...
		{		
			$("#hidden-invoice-country").html('<option value="ExportInvoice">Import Invoice (With IGST)</option><option value="ExportInvoiceWithoutIGST">Import Invoice (Without IGST)</option>');						
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='OK')
			{
				...
				if(data.gst == "")
				{
					$("select#reverse_Charge").val("1");
					$("select#reverse_Charge").attr("disabled", true).trigger("change");
				}
				else
				{
					
					$("select#reverse_Charge[data-value='']").val("0");
					$("select#reverse_Charge").attr("disabled", false).trigger("change");
				}
...
$(document).ready(function(){
	...
	$( "#product_invoice_form" ).validate({
		...
		submitHandler: function(form) {
			...
			if(validateCurrentRowsForSubmit() && checkValidDate() && checkValidPayment() )
			{
				$("select#reverse_Charge").attr("disabled", false);
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Purchase Invoice");
							}
