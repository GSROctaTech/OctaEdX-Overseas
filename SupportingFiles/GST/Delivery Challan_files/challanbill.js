...
rowdata += '		<input class="hidden-item-product-hsn" 		name="hidden-item-product-hsn[]" 		type="hidden" >';
...
rowdata += '		<input class="hidden-item-product-sellprice"	name="hidden-item-product-sellprice[]" type="hidden" >';
...
rowdata += '</div>';
rowdata += '</td>';
rowdata += '	<td class="price_field"><center>';
...
rowdata += '	</td></center>';
rowdata += '	<td class="discount_field price_field">';
...
rowdata += '	</td>';
rowdata += '<td  class="product_row_cgst_col tax_field">';
rowdata += '	<div class="inputlabel_wrapper">';
...
rowdata += '</td>';
rowdata += '	<td class="product_row_igst_col tax_field">';
...
rowdata += '	</td>';
rowdata += '	<td class="price_field"  style="width:90px;" ><center>';
	...
	if ( $("#hidden-invoice-country").is( "select" ) ) {
		if(country == "India")			
		{
			if(party_type == 'SEZ'){
				...
				$("#hidden-invoice-country").html('<option value="TaxInvoice">Regular</option><option value="JobWork">JobWork</option>');
			}		
...
function getInvoiceNumber(){
	...
	$.ajax({  
		...
		success: function(data){
			if(data.status=='OK')
			{
				$('[name="challan_id"]').val(data.lastid);
			}
		},
		error: function(data){
		},
...
function SetIGTS(){
	...
	if($("#hidden-invoice-country").val() == "SEZInvoiceWithoutIGST" ||  $("#hidden-invoice-country").val() == "ExportInvoiceWithoutIGST")
	{
		DisableALL_IGST();
	}
					...
					success: function(data) {
						...
						else
						{
							...
							if(data.errorTitle)
							{
								...
								$(".gogst_popup-header").html("Error in Saving Challan");
							}
							$('#gogst_popup').popup({
								scrolllock: true,
								transition: 'all 0.3s'
		...
		if( quantity != ""  )
			...
			if( disable_price_dchallan == 0 )
			{
				if( quantity != "" && rate != "" )
				{
					...
					}						if(disc>0 && discount_in=='rupee')			{				DiskRs = disc;			}
		...
		change: function(event, ui) {
			if (ui.item == null || ui.item == undefined) {
				...
				if (ui.item) {
					...
					$($targetparnt).find( ".hidden-item-product-hsn" ).val( ui.item.hsn);
					...
					$($targetparnt).find( ".hidden-item-product-sellprice" ).val( ui.item.sellprice);
...
function validateCurrentRows(){
	...
	$("#document-item-list .product-item-row").each(function(){
		...
		if(  rate == "" && disable_price_dchallan == 0 )
		{
