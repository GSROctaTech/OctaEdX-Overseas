...
#selFilesButton.setGeometry(30,30,60,30)
...
generateDataFromFileButton = QPushButton()
generateDataFromFileButton.setText('Call GenerateDataFromFile')
# layout.addWidget(findSelectedItemButton, 2, 1, 1, 2)
layout.addWidget(generateDataFromFileButton,0,4)   # Table Adding to 1 Row on Form/Window
generateDataFromFileButton.clicked.connect(lambda: testFileFunction())

viewTestData = QPushButton()
viewTestData.setText('Show Test Data')
# layout.addWidget(viewTestData, 2, 1, 1, 2)
layout.addWidget(viewTestData,0,5)   # Table Adding to 1 Row on Form/Window
...
# layout.addWidget(findSelectedItemButton, 2, 1, 1, 2)
layout.addWidget(viewDataOfSelFile,0,6)   # Table Adding to 1 Row on Form/Window
...
filestable.setStyleSheet("QHeaderView::section {background-color: rgb(100, 100, 100); color: rgb(200, 200, 200);}")
filestable.setStyleSheet("QTableCornerButton::section {background-color: rgb(100, 100, 100); color: rgb(200, 200, 200);}")
filestable.setStyleSheet("""
QTableWidget{background: #e1e1e1;selection-color: white;border: 1px solid lightgrey;color: #202020;outline: 0;
selection-background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #729fcf, stop: 1  #204a87);} 
QTableWidget::item::hover{background: qlineargradient(y1: 0, y2: 1, stop: 0 #babdb6, stop: 0.5 #d3d7cf, stop: 1 #babdb6);}
QTableWidget::item::focus{background: qlineargradient( y1: 0, y2: 1, stop: 0 #729fcf, stop: 1  #204a87);border: 0px;}
QHeaderView::section{background-color:#d3d7cf;color: #2e3436; font: bold}
QTableCornerButton::section{background-color:#d3d7cf;}""")


selFiletableCSS = "QTableWidget{background: #e1e1e1;selection-color: white;border: 1px solid lightgrey;color: #202020;outline: 0; " \
                  "selection-background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #729fcf, stop: 1  #204a87);}" \
                  "QTableWidget::item::hover{background: qlineargradient(y1: 0, y2: 1, stop: 0 #babdb6, stop: 0.5 #d3d7cf, stop: 1 #babdb6);}" \
                  "QTableWidget::item::focus{background: qlineargradient( y1: 0, y2: 1, stop: 0 #729fcf, stop: 1  #204a87);border: 0px;}" \
                  "QHeaderView::section{background-color:#d3d7cf;color: #2e3436; font: bold}" \
...
# QStatusBar{font-size: 7pt; color: #717171}
# QLineEdit{color: #484848;background-color: #fbfbfb;}
# QMenuBar{background: transparent;border: 0px;}
# QToolBar{background: transparent;border: 0px;}
# QPushButton{background: #d3d7cf ;}
# QMainWindow{background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,stop: 0 #E1E1E1, stop: 0.4 #DDDDDD,stop: 0.5 #D8D8D8, stop: 1.0 #D3D3D3);}
# QLineEdit{background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,stop: 0 #E1E1E1, stop: 0.4 #e5e5e5,stop: 0.5 #e9e9e9, stop: 1.0 #d2d2d2);}
# """

buttonCSS = "QPushButton {background-color: blue; height: 40px; border-radius: 5px;}" \
            "QPushButton:pressed {background-color: red; border-radius: 25px;}"\
            "QPushButton:hover {background: yellow;border:3px solid black;}" \
            "::item::hover"

selFilesButton.setStyleSheet(buttonCSS)
calcButton.setStyleSheet(buttonCSS)
getSelFilesButton.setStyleSheet(buttonCSS)
findSelectedItemButton.setStyleSheet(buttonCSS)
generateDataFromFileButton.setStyleSheet(buttonCSS)
viewTestData.setStyleSheet(buttonCSS)
viewDataOfSelFile.setStyleSheet(buttonCSS)
...
#filestable.setStyleSheet("::item:hover{background-color:rgba(222,100,123,100);} ::item:selected{background-color:rgba(100,200,100);}")
...
filestable.horizontalHeader().setSortIndicatorShown(True)
...
def showCustomMessage(mesg):
    ...
    try:
        buttonReply = QMessageBox.question(widget, 'PyQt5 message', mesg, QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel, QMessageBox.Cancel)
...
def contextMenuEvent(event):
    ...
    if selected_items:
        ...
        menu = QMenu()
        menu.setStyleSheet("""QMenu {border: 1px inset grey; background-color: #fff; color: #000; padding: 0;}
...
def testFileFunction():
    row = filestable.currentRow()
    fileName = (filestable.item(row, 0).text())
    dotIndex = fileName.index(".") + 1
    fileExtension = filestable.item(row, 0).text()[dotIndex:].upper()

    generateDataFromFile(fileName)

def generateDataFromFile(fileName):
    print('Entered generateDataFromFile()')

    try:
        # For Single & Current Selection Use the below
        showMessage("Input File Name : " + fileName)

        dotIndex = fileName.index(".") + 1
        fileExtension = fileName[dotIndex:].upper()
        showMessage("Input File Extension : " + fileExtension)

        confimReply = QMessageBox.question(widget, fileExtension + " File View",
                                           "Do you want to view " + fileExtension + " file data ",
                                           QMessageBox.Ok | QMessageBox.No, defaultButton=QMessageBox.Ok)
        if confimReply == QMessageBox.Ok:
            # Call respective method based on the file extension
            if fileExtension == 'CSV':
                showMessage("It's a " + fileExtension + " file")
            elif fileExtension in ('XLSX', 'XLS'):
                showMessage("It's a " + fileExtension + " file")
            elif fileExtension == 'XML':
                showMessage("It's a " + fileExtension + " file")
                ...
                QMessageBox.information(widget, "CSV File View",
                                        "You selected not to view " + fileExtension + " file data ",
                                        QMessageBox.Ok, defaultButton=QMessageBox.Ok)
        else:
            QMessageBox.information(widget, "CSV File View",
                                    "You selected not to view " + fileExtension + " file data ",
        ...
        print("From generateDataFromFile(fileName) Method")
        showException(" Occured in generateDataFromFile(fileName)")
...
def readExcel(excelFileName):  # -------------------------- Excel Files  -----------------------------------------------
  ...
  #------------------------ Copied Code Start Here ---------------------------------
        ...
        selFiletable.setStyleSheet(selFiletableCSS)
...
def readJSON(fileName):
    ...
    try:
        jsonfileName="http://universities.hipolabs.com/search?country=United+Kingdom"
        if jsonfileName:
            df = pd.read_json(jsonfileName, orient ='columns')  # , error_bad_lines=False
        ...
        showMessage(jsonfileName + " loaded")

    except Exception as error:
        print(str(error) + " Occured in readJSON Method")
        showException(" Occured in readJSON")
