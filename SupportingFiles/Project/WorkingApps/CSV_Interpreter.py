...
class MyWindow(QtWidgets.QWidget):
    ...
    def loadCsv(self, fileName):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Open CSV", (QtCore.QDir.homePath()),
                                                            "CSV (*.csv *.tsv)")
        if fileName:
            print(fileName)
            ff = open(fileName, 'r')
    ...
    def writeCsv(self, fileName):
        ...
        for rowNumber in range(self.model.rowCount()):
            fields = [self.model.data(self.model.index(rowNumber, columnNumber), QtCore.Qt.DisplayRole) for columnNumber
                      in range(self.model.columnCount())]
        ...
        for row in range(self.model.rowCount()):
            for column in range(self.model.columnCount()):
                ...
                if myitem is None:
                    ...
                    table = cursor.insertTable(model.rowCount(),
                                               model.columnCount())
